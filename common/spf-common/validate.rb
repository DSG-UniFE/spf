...
class Validate
  ...
  def self.conf?(opt)
    ...
    SERVICES_FOLDER = "gateway-module/src/main/java/ruby/spf-gateway/service-strategies"
    PROCESS_FOLDER = "gateway-module/src/main/java/ruby/spf-gateway/processing-strategies"
    ...
    opt.keys.each do |key|
      ...
      when "allow_services".to_sym
        services = Dir.glob(File.join(APP_FOLDER, "*")
      ...
      when  "service_policies".to_sym
        opt[key][:service_policies].keys.each do | service |
          ...
          when "find_text".to_sym
            process = Dir.glob(File.join(APP_FOLDER, "*")
