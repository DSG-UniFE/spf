    ...
    class LoopConnector
      ...
      def run(opts = {})
        if opts[:one_shot]
          ...
          begin
            ...
            logger.warn "*** #{LoopConnector.name} < #{@parent_class_name}: ONE-SHOT - connection attempt failed - server #{@host} down? ***"
          rescue => e
        ...
        while @keep_going.true?
          begin
            logger.info "*** #{LoopConnector.name} < #{@parent_class_name}: connection attempt ##{counter} to #{@host}:#{@port} ***"
            socket = Socket.tcp(@host, @port)
            handle_connection(socket, @host, @port)
            counter = 0
          rescue Errno::EBADF => e
            logger.warn "*** #{LoopConnector.name} < #{@parent_class_name}: connection attempt failed - server #{@host} down? waiting #{@reconnection_timeout}s before retrying ***"
          rescue SocketError => e
            ...
            logger.error "*** #{LoopConnector.name} < #{@parent_class_name}: connection attempt failed with an unexpected error - waiting #{@reconnection_timeout}s before retrying ***"
            logger.error e.class.inspect
      ...
      private
        ...
        def handle_connection(socket, host, port)
          raise "*** #{LoopConnector.name} < #{@parent_class_name}: parent class needs to implement the handle_connection method! ***"
        end
