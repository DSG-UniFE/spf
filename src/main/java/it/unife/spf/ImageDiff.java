...
public class ImageDiff
{
    ...
    */
    ...
    while (width1 % step > 0) width1--;
    int width2 = img2.getWidth(null);
    while (width2 % step > 0) width2--;
    int height1 = img1.getHeight(null);
    while (height1 % step > 0) height1--;
    int height2 = img2.getHeight(null);
    while (height2 % step > 0) height2--;
    ...
    for (int y = 0; y < height1; y += step) {
      for (int x = ((y/step) % 2 == 0) ? 0 : step/2 ; x < width1; x += step) {
        ...
        // Use square power to give more importance to big differences in luminance
        diff += (lum1 - lum2)*(lum1 - lum2);
      }
    }
    
    // The number of pixels processed is X * Y minus 1 for each odd line (where we begin from 4)
