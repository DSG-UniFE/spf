...
//import org.opencv.highgui.Highgui; // for opencv2.4.11
...
public class TextRecognition {
	...
	public static String doOCR(String file){
		...
		//Imgcodecs.imwrite("img-water-grayscale.jpg", source);
		//Imgproc.cvtColor(source, source, Imgproc.COLOR_BGR2GRAY);
		Mat destination = new Mat(source.rows(),source.cols(),source.type());
		Imgproc.threshold(source,destination, 0, 255 ,Imgproc.THRESH_OTSU);
		//Imgproc.adaptiveThreshold(destination, new Mat(), 255, Imgproc.ADAPTIVE_THRESH_GAUSSIAN_C, Imgproc.THRESH_BINARY, 11, 2);
		...
		ArrayList<RotatedRect> areas = new ArrayList<RotatedRect>();
		 for (MatOfPoint contour : contours) {
			...
			//Discard very small boxes
			 ...
			 if(box.angle < -45.0){proportion = box.size.height / box.size.width;}
			 else {proportion = box.size.width / box.size.height;}
		 ...
		 for (RotatedRect rect : areas) {
			...
			Imgproc.rectangle(img, new Point(brect.x, brect.y), new Point(brect.x + brect.width, brect.y + brect.height),  new Scalar(110, 220, 0), 5);
	        ...
	        Imgproc.adaptiveThreshold(sub_image, adaptive, 255, Imgproc.ADAPTIVE_THRESH_GAUSSIAN_C, Imgproc.THRESH_BINARY, 11, 2);
		 ...
		 //FaceRecognition.doFaceRec("street-crowd.jpg");
		 System.out.println(response);	 
		 return response;
		 //Imgcodecs.imwrite("text-recognition-dilated-block-segmentation.jpg", img);
